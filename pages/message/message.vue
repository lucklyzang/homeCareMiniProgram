				...
				<view class="message-content">
					<view class="message-content-top">
						...
						<view class="message-date">
							<text>
								{{ getNowFormatDate(new Date(item.lastTime),4) }}
							</text>
						</view>
					</view>
...
<script>
	...
	import { notifyMessageSummary, notifySummary, latestNews, getUserChatList, chatMessageRead, updateInformationRead, updateNotifyAllRead, updateNotifymessageAllRead } from '@/api/user.js'
	...
	export default {
		...
		methods: {
			...
			updateInformationReadEvent() {
				updateInformationRead({terminal: 'USER'}).then((res) => {
					if ( res && res.data.code == 0) {
			...
			updateNotifyAllReadEvent() {
				updateNotifyAllRead({terminal: 'USER'}).then((res) => {
					if ( res && res.data.code == 0) {
