<template>
	<view class="content-box">
		...
		<view class="top-area-box">
			<view class="nav">
				<nav-bar :home="false" backState='3000' bgColor="none" title="通知消息" @backClick="backTo">
				</nav-bar> 
		  </view>
		</view>
		<view class="message-list-wrapper">
			<u-empty text="暂无通知消息" mode="list" v-if="isShowNoData"></u-empty>
...
<script>
	...
	import { notifyMessageSummaryPage, updateNotifymessageRead } from '@/api/user.js'
	export default {
		components: {
			navBar
		},
		...
		onLoad() {
			this.queryNotifyMessageSummaryPageList({
		...
		methods: {
			...
			enterLatestNewsDetailsEvent (item,innerItem) {
				this.updateNotifymessageReadEvent(innerItem.id);
				// 传递通知详情内容
				let mynavData = encodeURIComponent(JSON.stringify(innerItem));
				uni.navigateTo({
					url: '/messagePackage/pages/notifyMessageListDetails/notifyMessageListDetails?transmitData='+mynavData
				})
			},
			
			// 获取通知列表
			queryNotifyMessageSummaryPageList (data,flag) {
				if (flag) {
				...
				this.noticeList = [];
				notifyMessageSummaryPage(data).then((res) => {
			...
			// 更新通知为已读
			updateNotifymessageReadEvent(id) {
				this.showLoadingHint = true
				this.infoText = '';
				updateNotifymessageRead(id).then((res) => {
					if ( res && res.data.code == 0) {
					} else {
						this.$refs.uToast.show({
							message: res.data.msg,
